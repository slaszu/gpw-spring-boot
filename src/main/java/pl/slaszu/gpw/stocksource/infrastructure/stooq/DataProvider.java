...
public class DataProvider implements DataProviderInterface {
    ...
    public List<StockDTO> getData() throws FetchStocksException {
        ...
        for (Element row : rows) {
            ...
            stockCodes.add(cells.get(0).text());
        }

        log.info(stockCodes.toString());

        return stockCodes.stream().map((String code) -> new StockDTO(code)).toList();
    }
